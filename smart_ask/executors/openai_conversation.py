...
def _input(request: ConversationRequest) -> list[dict[str, Any]]:
    ...
    for message in request.messages:
        if message.role not in ("user", "assistant", "system", "developer"):
            raise UnsupportedConversationFeature(
                f"unsupported OpenAI Responses message role: {message.role!r}"
            )
        ordinary = tuple(
            block
        ...
        if unsupported:
            raise UnsupportedConversationFeature(
                f"OpenAI Responses cannot encode block types: {unsupported}"
            )
        if ordinary:
            values.append({
                "type": "message",
    ...
    if not values:
        raise UnsupportedConversationFeature(
            "OpenAI Responses requires at least one encodable input item"
        )
    return values
...
class OpenAIConversationExecutor:
    ...
    async def stream(
        ...
        async with self._client.stream(
            ...
            if response.is_error:
                ...
                raise RuntimeError(
                    f"OpenAI Responses API returned {response.status_code}: {message}"
                )
            async for line in response.aiter_lines():
                ...
                if not isinstance(event, Mapping):
                    raise ValueError("OpenAI stream events must be JSON objects")
                ...
                if event_type == "response.output_item.added":
                    ...
                    if not isinstance(item_id, str) or not item_id:
                        raise ValueError("OpenAI function calls require an item id")
                    if not isinstance(call_id, str) or not call_id:
                        raise ValueError("OpenAI function calls require a call id")
                    if not isinstance(name, str) or not name:
                        raise ValueError("OpenAI function calls require a name")
                ...
                if event_type == "response.failed":
                    ...
                    raise RuntimeError(
                        _response_error(details, "OpenAI response failed")
                    )
                if event_type == "error":
                    raise RuntimeError(_response_error(event, "OpenAI stream failed"))

        if not started:
            raise ValueError("OpenAI stream ended without any response events")
