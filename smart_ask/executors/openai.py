...
def _count(value, name: str) -> int | None:
    ...
    if isinstance(value, bool) or not isinstance(value, Integral) or value < 0:
        raise ValueError(f"OpenAI response {name} must be a non-negative integer")
    return int(value)
...
class OpenAIExecutor:
    ...
    def execute(self, request: ExecutionRequest) -> ModelResult:
        ...
        if (
            ...
            raise ValueError(
                "OpenAI response reasoning_tokens cannot exceed output_tokens"
            )
        if (
            ...
            raise ValueError(
                "OpenAI response cached_tokens cannot exceed input_tokens"
            )
