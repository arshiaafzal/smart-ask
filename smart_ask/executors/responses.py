"""Provider-neutral Responses API transport mechanics."""
...
def _input(request: Conversation) -> list[dict[str, Any]]:
...
def _instructions(request: Conversation) -> str | None:
...
def _usage(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(output_details, Mapping):
        ...
        if (
            ...
            result["visible_output_tokens"] = output - reasoning
    return result
...
class ResponsesTransport:
    ...
    async def stream(
        ...
        if not terminal:
            raise RuntimeError("Responses API stream ended without terminal evidence")
