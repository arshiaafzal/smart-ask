...
def _optional_non_negative_integer(value, field_name: str) -> int | None:
    if value is _MISSING or value is None:
        return None
    if isinstance(value, bool) or not isinstance(value, Integral) or value < 0:
        raise ValueError(
            f"OpenRouter response {field_name} must be a non-negative integer"
        )
    return int(value)


def _optional_non_negative_number(value, field_name: str) -> float | None:
    if value is _MISSING or value is None:
        return None
    if (
        ...
        raise ValueError(
            f"OpenRouter response {field_name} must be finite and non-negative"
        )
    return float(value)


def _usage_detail(usage, group_name: str, field_name: str) -> int | None:
    if usage is None:
        return None
...
def _finish_evidence(choice, refusal: str | None) -> tuple[str, str | None]:
    ...
    for field_name, value in (
        ...
        if value is not _MISSING and value is not None and (
            ...
            raise TypeError(
                f"OpenRouter response {field_name} must be text or None"
            )
...
class OpenRouterExecutor:
    ...
    def execute(self, request: ExecutionRequest) -> ModelResult:
        ...
        if message is _MISSING or message is None:
            raise ValueError("OpenRouter response choice must contain a message")
        content = _field(message, "content")
        if content is _MISSING:
            raise ValueError("OpenRouter response choice must contain a message")
        if content is not None and not isinstance(content, str):
            raise TypeError("OpenRouter response message content must be text or None")
        text = content or ""
        refusal = _field(message, "refusal")
        if refusal is _MISSING or refusal is None:
            refusal = None
        elif not isinstance(refusal, str):
            raise TypeError("OpenRouter response message refusal must be text or None")
        elif not refusal.strip():
            raise ValueError(
                "OpenRouter response message refusal must be non-empty or None"
            )
        finish_reason, native_finish_reason = _finish_evidence(choice, refusal)
        ...
        if completion_tokens is not None and reasoning_tokens is not None:
            if reasoning_tokens > completion_tokens:
                raise ValueError(
                    "OpenRouter response reasoning_tokens cannot exceed "
                    "completion_tokens"
                )
        ...
        for field_name, detail in (
            ...
            if (
                ...
                raise ValueError(
                    f"OpenRouter response {field_name} cannot exceed "
                    "prompt_tokens"
                )
